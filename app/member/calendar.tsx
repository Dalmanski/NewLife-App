  ...
  return (
    <View style={{ flex: 1, backgroundColor: "#fff" }}>
      <ScrollView contentContainerStyle={{ padding: 20, gap: 12 }}>
        <Text style={{ fontSize: 24, fontWeight: "700" }}>{memberName}'s Calendar</Text>
          ...
          {Array.from({ length: days.length / 7 }).map((_, row) => (
            <View key={row} style={{ flexDirection: "row", minHeight: 92 }}>
              {days.slice(row * 7, row * 7 + 7).map((day, index) => {
                ...
                const isToday = date === iso(new Date().getFullYear(), new Date().getMonth(), new Date().getDate());
                ...
                return (
                  ...
                  >
                    ...
                    {tasksForDay.length > 2 ? <Text style={{ fontSize: 11 }}>+{tasksForDay.length - 2} more</Text> : null}
                  </Pressable>
                );
              })}
            </View>
      ...
      <Modal visible={!!selectedDate} transparent animationType="fade" onRequestClose={() => setSelectedDate(null)}>
        ...
        >
          ...
          >
            <Text style={{ fontSize: 20, fontWeight: "700", marginBottom: 8 }}>{selectedLabel}</Text>
